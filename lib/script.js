...
(function () {
  ...
  function createMap(us, map, type) {
    ...
    function getLegendDetails() {
      ...
      if (type === 'default') {
        legendData = ["State has taken fewer than 10 actions to improve maternal health conditions", "State has taken 10-15 actions to improve maternal health conditions", "State has taken 16-20 actions to improve maternal health conditions", "State has taken more than 20 actions to improve maternal health conditions"];
        legendCol = ["#d3f4f4", "#9bdddd", "#1894ac", "#02578c"];
      } else if (type === 'Payment Reform') {
        legendData = ["Has not implemented any payment reform actions", "Implemented 1 payment reform action", "Implemented 2 payment reform actions", "Implemented 2 payment reform actions"];
        ...
        if (selectedPolicy === "Payment Reform") {
          ...
          for (var _key in allData.get(selectedPolicy).get(selectedState)) {
            if (_key.indexOf("Value") >= 0) {
              ...
              policyVals.append('p').text(policyVal);
            }

            if (_key === "Status") {
              policyVals.append('p').text(allData.get(selectedPolicy).get(selectedState)[_key]);
            }
